...
<script>
    layui.use(['table', 'form'], function () {
        ...
        table.render({
            ...
            , height: 523
            , url: '/sys/course.html?act=table' //数据接口
            , page: true //开启分页
            , cols: [[ //表头
            ...
            layer.open({
                ...
                , yes: function (index, layero) {
                    ...
                    for (var i = 0; i < nodes.length; i++) {
                        roleIds.push(nodes[i].stuId)
                        console.log(nodes[i].stuId)
                    }

                    $.ajax({
                        url: "/sys/student.html?act=assign",
                        method: "post",
                        data: "stuId="+stuId+"&roleIds="+roleIds,
                        success: function (res) {
                            if (res.status) {
                                layer.close(index);
                            } else {
                ...
                , btn2: function (index, layero) {
                    ...
                    $.ajax({
                        url: "sys/student.html?act=roleIds",
                        data:"userId="+id,
                        success:function (res) {
                            $('#menu-tree').tree({//打开成功后加载树
                                url: "sys/student.html?act=roleTree",//数据接口
                                checkbox: true,
...
<script type="text/html" id="editFormlayer">
    <form class="layui-form" style="width:80%;padding-top: 20px" id="editForm" lay-filter="editForm">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="teacher" required lay-verify="required" placeholder="请输入教师工号" autocomplete="off"
                       class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
